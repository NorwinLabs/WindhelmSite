...
;
(function () {
        const DISCORD_SERVER_ID = "974051907058405467";
        ...
        async function fetchDiscordStats() {
          try {
            ...
            console.log("Discord API fetch error:", error);
            // Fallback values
            if (memberCountElement) {
              memberCountElement.textContent = "50+";
              const memberLabel = document.getElementById(
                "discord-member-label",
              );
              if (memberLabel) memberLabel.textContent = " members";
            }
          }
        }
        ...
        function updateUI(data) {
          if (memberCountElement) {
            // Total members (approximate from presence)
            const totalMembers =
              data.presence_count || data.members?.length || 0;
            memberCountElement.textContent =
              totalMembers.toLocaleString() + "+";
            const memberLabel = document.getElementById("discord-member-label");
            if (memberLabel) memberLabel.textContent = " members";

            // Also update contact section if it exists
            const contactMemberCount = document.getElementById(
              "discord-member-count-contact",
            );
            if (contactMemberCount) {
              contactMemberCount.textContent =
                totalMembers.toLocaleString() + "+";
            }
          }
        }

        // Fetch on page load (deferred)
        if (memberCountElement) {
          // Use requestIdleCallback for non-critical data
          if ("requestIdleCallback" in window) {
            requestIdleCallback(() => fetchDiscordStats());
          } else {
            setTimeout(fetchDiscordStats, 100);
          }
          // Refresh every 5 minutes
          setInterval(fetchDiscordStats, CACHE_DURATION);
        }
      })();
;
// Hero background video — random selection
      (function () {
        ...
        if (
          ...
          video = null; // keep the poster image only
        }
        if (video && HERO_VIDEOS.length > 0) {
          var src = HERO_VIDEOS[Math.floor(Math.random() * HERO_VIDEOS.length)];
          video.src = src;
          video.load();
          video.play().catch(function () {}); // silently handle autoplay policy
        }
      })();
        ...
        listEl.innerHTML = updates
          .map((update) => {
            ...
            if (bbSrcMatch) {
              ...
              imgUrl = bbPlainMatch[1]
                .trim()
                .replace("{STEAM_CLAN_IMAGE}", CLAN_IMG);
      ...
      function showSteamError() {
        ...
        listEl.innerHTML = `
          <div style="text-align: center; padding: 3rem; color: #94a3b8;">
            <p style="margin-bottom: 1rem; font-size: 1.1rem;">No updates available right now.</p>
