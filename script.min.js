...
// ===== LIVE DISCORD STATS =====
(function () {
  // IMPORTANT: Replace this with your actual Discord Server ID
  // To get your server ID:
  // 1. Enable Developer Mode in Discord (User Settings > Advanced > Developer Mode)
  // 2. Right-click your server icon and click "Copy ID"
  // 3. Also enable the widget in Server Settings > Widget > Enable Server Widget
  ...
  if (!membersElement || !onlineElement) {
    console.log("Discord stat elements not found");
    return;
  }

  function fetchDiscordStats() {
    console.log("Fetching Discord stats...");
    console.log("Server ID:", DISCORD_SERVER_ID);
    console.log("Server ID type:", typeof DISCORD_SERVER_ID);
    
    var url = "https://discord.com/api/guilds/" + DISCORD_SERVER_ID + "/widget.json";
    ...
    fetch(url)
      .then(function (response) {
        if (!response.ok) {
          if (response.status === 403) {
            throw new Error(
              "Discord Widget is not enabled. Go to Server Settings → Widget → Enable Server Widget",
            );
          } else if (response.status === 404) {
            throw new Error("Server not found. Check your Discord Server ID.");
          } else {
      ...
      .then(function (data) {
        ...
        var onlineCount = data.presence_count || 0;
        var totalMembers = data.members ? data.members.length : onlineCount;

        // Format numbers with commas
        membersElement.textContent = totalMembers.toLocaleString();
        onlineElement.textContent = onlineCount.toLocaleString();

        console.log(
          `✅ Discord: ${totalMembers} members, ${onlineCount} online`,
        );
      })
      .catch(function (error) {
        console.error("❌ Failed to fetch Discord stats:", error.message);
        console.log("\n" + "=".repeat(60));
        console.log("📝 HOW TO FIX DISCORD STATS:");
        console.log("=".repeat(60));
        console.log("1. Open Discord Desktop/Web");
        console.log("2. Click your server name → Server Settings");
        console.log("3. Scroll down and click 'Widget' in the left sidebar");
        console.log("4. Toggle 'Enable Server Widget' to ON (blue)");
        console.log("5. Click 'Save Changes' at the bottom");
        console.log("6. Wait 10 seconds, then refresh this page");
        console.log("=".repeat(60) + "\n");

        console.log("🔍 Current Server ID:", DISCORD_SERVER_ID);
        console.log(
          "🔗 Testing URL:",
        ...
        console.log(
          "\n💡 TIP: Open the testing URL above in a new tab to see the exact error.\n",
        );

        // Show placeholder if widget is not enabled
        membersElement.textContent = "—";
        onlineElement.textContent = "—";

        // Keep stats visible with dashes to show it's trying to load
      });
  }
