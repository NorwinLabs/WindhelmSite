...
// The blog section is rendered statically in index.html so there is always
// real, crawlable content on first paint (no loading spinner, no error
// state visible to users or to Googlebot). This function only enhances
// that static content in the background with live Steam News API posts
// when it can reach them — it never removes or blocks the static markup.
async function loadSteamUpdates() {
  const listEl = document.getElementById("updates-list");
  if (!listEl) return;

  const CACHE_KEY = "steam-news-cache-v6";
  ...
  try {
    ...
    // Try our own Cloudflare Worker (blog-backend/steam-news-worker.js) and
    // a third-party CORS proxy in parallel, and use whichever answers first
    // with valid data — keeps the worst case fast instead of stacking two
    // sequential timeouts.
    const proxyUrls = [
    ...
    const attempts = proxyUrls.map(async (proxyUrl) => {
      ...
      if (!resp.ok) throw new Error(`Bad response from ${proxyUrl}`);
      const data = await resp.json();
      if (!data?.appnews) throw new Error(`No appnews from ${proxyUrl}`);
      return data;
    });
    ...
    // Live fetch failed (or the worker is unreachable) — the static blog
    // posts already rendered in the page stay exactly as they are.
    console.log("Steam news unavailable, keeping static blog content:", error.message);
  }
}
...
// No longer needed: extractSteamUpdates, showSteamError (the blog section
// now ships real static content directly in index.html, so there is
// nothing to fall back to on error — see loadSteamUpdates() above).
